...
pub fn get_glwe_avg_err<Scalar, KeyCont, InputCont, PtCont>(
    glwe_secret_key: &GlweSecretKey<KeyCont>,
    input: &GlweCiphertext<InputCont>,
    correct_val_list: &PlaintextList<PtCont>,
) -> f64
    ...
    PtCont: Container<Element=Scalar>,
{
    ...
    (avg_err as f64)/(polynomial_size as f64)
}
...
pub fn get_abs_err_ggsw_bit<Scalar: UnsignedTorus>(
    glwe_secret_key: &GlweSecretKeyOwned<Scalar>,
    ggsw_in: GgswCiphertextView<Scalar>,
    correct_val: Scalar,
) -> f64 {
    ...
    let total_coeffs =  polynomial_size * (glwe_size.0 * decomp_level);
    ...
    let sum_avg_err : u128 = Scalar::cast_into(avg_err);

    (sum_avg_err as f64)/(total_coeffs as f64)
}
